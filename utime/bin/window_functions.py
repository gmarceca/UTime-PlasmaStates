...
def get_transitions_in_window(time_window):
    
    # return (np.asarray([time_window.lh_trans.values, time_window.hl_trans.values, time_window.hd_trans.values,
    #         time_window.dh_trans.values, time_window.ld_trans.values, time_window.dl_trans.values,
    #         time_window.no_trans.values]))
    vals = time_window[trans_ids]
    # print(vals.shape)
...
#     return np.asarray([time_window.elm, time_window.no_elm])
...
def get_states_in_window(time_window):
    ...
    # n, l, d, h = time_window.sm_none_label.values, time_window.sm_low_label.values, time_window.sm_dither_label.values, time_window.sm_high_label.values
    # for k in range(len(l)):
    #     no, low, dither, high =n[k], l[k], d[k], h[k]
    #     # print(round(no + low+dither+high, 3))
    #     assert(round(no + low+dither+high, 3)==1)
    # return np.asarray([n, l, d, h])
...
# def get_dithers_in_window(window):
#     return np.asarray([window.dither, window.no_dither]) #this gets the dither/non dither target for the CNN, which is input for the RNN
...
# 
# def get_dither_in_dt(current_dt):
#     return (current_dt.dither, current_dt.no_dither)

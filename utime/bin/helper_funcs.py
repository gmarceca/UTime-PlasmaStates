...
class Path():
    ...
    def equal_state(self, other):
        if self.states[-1] == other.states[-1]:
            return True
        else:
            return False
...
def reset_labels(df):
    # L = 1, D = 2, H = 3
    df.loc[df.LHD_label == 2, 'LHD_label'] = 1
    df.loc[df.LHD_label == 3, 'LHD_label'] = 2
    return df

def remove_conv_start_end(df):
    return df[20 : len(df)-20]

def normalize_signals_mean(df, machine, func='minmax'):
    if func=='minmax':
        try:
            df['TDAI'] = (df.TDAI.values - np.min( df.TDAI.values))/(np.max(df.TDAI.values) -  np.min(df.TDAI.values))
        except AttributeError:
            print('TDAI signal not present in dataframe')
        try:
            df['FIR'] = (df.FIR.values - np.min( df.FIR.values))/(np.max(df.FIR.values) -  np.min(df.FIR.values))
        except AttributeError:
            print('FIR signal not present in dataframe')
        try:
            df['PD'] = (df.PD.values - np.min( df.PD.values))/(np.max(df.PD.values) -  np.min(df.PD.values))
        except AttributeError:
            print('PD signal not present in dataframe')
        try:
            df['WP'] = (df.WP.values - np.min( df.WP.values))/(np.max(df.WP.values) -  np.min(df.WP.values))
        except AttributeError:
            print('WP signal not present in dataframe')
        try:
            df['TP135'] = (df.TP135.values - np.min( df.TP135.values))/(np.max(df.TP135.values) -  np.min(df.TP135.values))
        except AttributeError:
            print('TP135 signal not present in dataframe')
        try:
            df['DML'] = (df.DML.values - np.min( df.DML.values))/(np.max(df.DML.values) -  np.min(df.DML.values))
        except AttributeError:
            print('DML signal not present in dataframe')
    return df
...
def load_fshot_from_labeler(shot_id, data_dir, machine):
    shot, labeler = shot_id.split('-')
    
    fshot = pd.read_csv(data_dir + labeler + '/' + machine + '_' + str(shot) + '_' + labeler + '_labeled.csv')
    if machine == 'TCV':
        fshot = remove_current_30kA(fshot)
    elif machine == 'JET':
        fshot = remove_current_750kA(fshot)
...
def load_fshot_from_number(shot_num, data_dir):
    labeler= 'ffelici' #could be another labeler as well, it doesnt matter since we only want the signals.
    return load_fshot_from_labeler(str(shot_num)+'-'+labeler, data_dir)

def load_fshot_from_classifier(shot_id, data_dir):
    shot, labeler = shot_id.split('-')
    print(shot, labeler, data_dir + labeler)
    fshot = pd.read_csv(data_dir + labeler + '/TCV_'  + str(shot) + '_LSTM_det.csv')
...
def load_shot_and_equalize_times(data_dir, shot, labelers, machine):
    print('Reading shot', shot)
    fshot = pd.read_csv(data_dir + labelers[0] + '/' + machine + '_'  + str(shot) + '_' + labelers[0] + '_labeled.csv', encoding='utf-8')
    shot_df = fshot.copy()
    shot_df.LHD_label = [int(i) for i in shot_df.LHD_label.values]
    if machine == 'TCV':
        shot_df = remove_current_30kA(shot_df)
    elif machine == 'JET':
        shot_df = remove_current_750kA(shot_df)
    ...
    return shot_df, intersect_times

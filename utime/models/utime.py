...
from tensorflow.keras.layers import Input, BatchNormalization, Dropout, Cropping2D, \
...
class UTime(Model):
    ...
    def create_encoder(in_,
        ...
        for i in range(depth):
            ...
            conv = Conv2D(filters, (kernel_size, 1),
                          ...
                          name=l_name + "_conv1", trainable=trainable)(in_)
            bn = BatchNormalization(name=l_name + "_BN1", trainable=trainable)(conv)
            conv = Conv2D(filters, (kernel_size, 1),
                          activation=activation, padding=padding,
                          kernel_regularizer=kernel_reg,
                          dilation_rate=dilation,
                          name=l_name + "_conv2", trainable=trainable)(bn)
            bn = BatchNormalization(name=l_name + "_BN2", trainable=trainable)(conv)
        ...
        conv = Conv2D(filters, (kernel_size, 1),
                      ...
                      kernel_regularizer=kernel_reg,
                      dilation_rate=1,
                      name=name + "_conv1", trainable=trainable)(in_)
        bn = BatchNormalization(name=name + "_BN1", trainable=trainable)(conv)
        conv = Conv2D(filters, (kernel_size, 1),
                      activation=activation, padding=padding,
                      kernel_regularizer=kernel_reg,
                      dilation_rate=1,
                      name=name + "_conv2", trainable=trainable)(bn)
        encoded = BatchNormalization(name=name + "_BN2", trainable=trainable)(conv)
    ...
    def create_upsample(self,
        ...
        for i in range(depth):
            ...
            conv = Conv2D(filters, (fs, 1),
                          ...
                          name=l_name + "_conv1", trainable=trainable)(up)
            bn = BatchNormalization(name=l_name + "_BN1", trainable=trainable)(conv)
            ...
            conv = Conv2D(filters, (kernel_size, 1),
                          activation=activation, padding=padding,
                          kernel_regularizer=kernel_reg,
                          name=l_name + "_conv2", trainable=trainable)(merge)
            bn = BatchNormalization(name=l_name + "_BN2", trainable=trainable)(conv)
            conv = Conv2D(filters, (kernel_size, 1),
                          activation=activation, padding=padding,
                          kernel_regularizer=kernel_reg,
                          name=l_name + "_conv3", trainable=trainable)(bn)
            in_ = BatchNormalization(name=l_name + "_BN3", trainable=trainable)(conv)
        return in_
    ...
    def init_model(self, inputs=None, create_seg_modeling=True, name_prefix="", name_prefix_lastLayer=""):
        """
        ...
        cls = self.create_dense_modeling(in_=up,
                                         ...
                                         name_prefix=name_prefix_lastLayer)

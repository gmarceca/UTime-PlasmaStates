...
class ValidationSequence:
    ...
    def __init__(self, sequences, no_log=False, logger=None):
        ...
        print('type sequences: ', type(self.sequences))
        #self.IDs = {{list(x)[0].split('/')[0] for x in val} for k, val in sequences[0].LabelIDs.items()}
        
        self.IDs = ['dataset_1']
        
        self.batch_size = self.sequences[0].batch_size
        print('self.batch_size val: ', self.batch_size)
        
        self.n_classes = self.sequences[0].no_classes
    ...
    #    Returns the batch size as output by this ValidationMultiSequence
    #    """
    #    return self.sequences.batch_size
